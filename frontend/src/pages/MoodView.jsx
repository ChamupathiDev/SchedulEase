                ...
                <ul className="mt-2">
                  {proposedSchedules.map((item) => (
                    <li key={item._id} className="text-sm text-gray-700">
                      {item.moduleName}:{" "}
                      {new Date(item.oldStartTime).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})} -{" "}
                      {new Date(item.oldEndTime).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})} will change to{" "}
                      {new Date(item.proposedStartTime).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})} -{" "}
                      {new Date(item.proposedEndTime).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}
                    </li>
                  ))}
                </ul>
